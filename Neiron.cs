         ...
         public double GetRes(int[,] data){
             ...
             for (int n = 0; n < veight.GetLength(0); n++)
                 for (int m = 0; m < veight.GetLength(1); m++)
                     res += 1 - Math.Abs(veight[n, m] - data[n, m]);  //  res = 1 - Math.Abs(veight[n, m] - data[n, m])+res
                                                                      // в этой строке мы считаем отклонения 
                                                                      // каждого элемента входного массива от 
                                                                      // усреднённого значения из памяти
              //veight.GetLength=10; veight.GetLength(1)=10 "бесконечность" = 58.6
         ...
         public int Training(int[,] data)
         {
             ...
             for (int n = 0; n < veight.GetLength(0); n++)
                 for (int m = 0; m < veight.GetLength(1); m++)
                 {
                     ...
                     veight[n, m] += 2 * (v - 0.5f) / countTrainig;  //  veight[0, 0] = (2 * (v - 0.5f) / countTrainig) + veight[0, 0]
